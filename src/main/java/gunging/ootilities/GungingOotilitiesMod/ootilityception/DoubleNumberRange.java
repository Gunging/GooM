...
public class DoubleNumberRange extends QuickNumberRange<Double> {
     ...
     * @since 1.0.0
     */
    @Contract("null -> null") @Nullable public static DoubleNumberRange getFromString(@Nullable String qnr) {
        ...
        if (qnr.equals("..")) { return new DoubleNumberRange(null, null); }
        if (qnr.equals("*")) { return new DoubleNumberRange(null, null); }

        // Number itself? So it's basically an EXACTLY this value, I'll allow it.
        Double asItself = OotilityNumbers.DoubleParse(qnr);
        if (asItself != null) { return new DoubleNumberRange(asItself, asItself); }
